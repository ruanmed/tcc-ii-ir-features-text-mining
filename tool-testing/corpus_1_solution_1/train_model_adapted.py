...
logging.basicConfig(level=logging.INFO,
                    ...
                    datefmt='%Y-%m-%d %H:%M:%S',
                    filename=f'logs/C1-S1-{datestr}-{variable_name}_debug.log',
                    filemode='w')
...
handler1 = logging.FileHandler(f'logs/C1-S1-{datestr}-{variable_name}.log')
...
exp_dict = {
    ...
    'tool': tool,
    'db': 'authorprof',
    'db_name': authorprof_db_name,
    'add_ir_variables': add_ir_variables,
    'solution_number': '1',
    'solution_name': '2_daneshvar18',
    'train_data_folder': TRAIN_DATA_ROOT_FOLDER,
    'test_data_folder': TEST_DATA_ROOT_FOLDER,
    'ir_top_k': ir_top_k,
...
testTool = IndexToolManager(
    indexName=str(authorprof_db_name), top_k=exp_dict['ir_top_k'])
...
def load_datasets_development(preset_key):
    ...
    PRESETS_DICTIONARY = {'PAN18_English': {'dataset_name': 'PAN 2018 English',
                                            'xmls_directory': TRAIN_DATA_ROOT_FOLDER + 'en/text',
                                            'truth_path': TRAIN_DATA_ROOT_FOLDER + 'en/en.txt',
                                            'txts_destination_directory': TRAIN_DATA_ROOT_FOLDER + 'TXT Files/en',
                                            },
                          'PAN18_Spanish': {'dataset_name': 'PAN 2018 Spanish',
                                            'xmls_directory': TRAIN_DATA_ROOT_FOLDER + 'es/text',
                                            'truth_path': TRAIN_DATA_ROOT_FOLDER + 'es/es.txt',
                                            'txts_destination_directory': TRAIN_DATA_ROOT_FOLDER + 'TXT Files/es',
                                            },
                          'PAN18_Arabic': {'dataset_name': 'PAN 2018 Arabic',
                                           'xmls_directory': TRAIN_DATA_ROOT_FOLDER + 'ar/text',
                                           'truth_path': TRAIN_DATA_ROOT_FOLDER + 'ar/ar.txt',
                                           'txts_destination_directory': TRAIN_DATA_ROOT_FOLDER + 'TXT Files/ar',
                                           },
                          }
    PRESET = PRESETS_DICTIONARY[preset_key]

    # Load the PAN 2018 training dataset and the truth from the files into lists
    logger.info("Loading the %s training dataset and the truth...",
                PRESET['dataset_name'])
    merged_tweets_of_authors, truths, author_ids, original_tweet_lengths =\
        process_data_files.load_pan_data(PRESET['xmls_directory'], PRESET['truth_path'],
                                         False, PRESET['txts_destination_directory'], exp_dict=exp_dict, exec_type='training')
...
def load_datasets_test(preset_key):
    """Load the PAN dataset for the development phase.
    ...
    PRESETS_DICTIONARY = {'PAN18_English': {'dataset_name': 'PAN 2018 English',
                                            'xmls_directory': TRAIN_DATA_ROOT_FOLDER + 'en/text',
                                            'truth_path': TRAIN_DATA_ROOT_FOLDER + 'en/en.txt',
                                            'test_xmls_directory': TEST_DATA_ROOT_FOLDER + 'en/text',
                                            'test_truth_path': TEST_DATA_ROOT_FOLDER + 'en/en.txt',
                                            'txts_destination_directory': TRAIN_DATA_ROOT_FOLDER + 'TXT Files/en',
                                            },
                          'PAN18_Spanish': {'dataset_name': 'PAN 2018 Spanish',
                                            'xmls_directory': TRAIN_DATA_ROOT_FOLDER + 'es/text',
                                            'truth_path': TRAIN_DATA_ROOT_FOLDER + 'es/es.txt',
                                            'test_xmls_directory': TEST_DATA_ROOT_FOLDER + 'es/text',
                                            'test_truth_path': TEST_DATA_ROOT_FOLDER + 'es/es.txt',
                                            'txts_destination_directory': TRAIN_DATA_ROOT_FOLDER + 'TXT Files/es',
                                            },
                          'PAN18_Arabic': {'dataset_name': 'PAN 2018 Arabic',
                                           'xmls_directory': TRAIN_DATA_ROOT_FOLDER + 'ar/text',
                                           'truth_path': TRAIN_DATA_ROOT_FOLDER + 'ar/ar.txt',
                                           'test_xmls_directory': TEST_DATA_ROOT_FOLDER + 'ar/text',
                                           'test_truth_path': TEST_DATA_ROOT_FOLDER + 'ar/ar.txt',
                                           'txts_destination_directory': TRAIN_DATA_ROOT_FOLDER + 'TXT Files/ar',
                                           },
                          }
    PRESET = PRESETS_DICTIONARY[preset_key]
    ...
    logger.info("Loading the %s training dataset and truth...",
                PRESET['dataset_name'])
    docs_train, y_train, author_ids_train, original_tweet_lengths_train, train_ir_vars = \
        process_data_files.load_pan_data(
            PRESET['xmls_directory'], PRESET['truth_path'], False, None, exp_dict=exp_dict, exec_type='training')

    # Load the PAN 2018 test dataset from the files into lists
    logger.info("Loading the %s test dataset...", PRESET['dataset_name'])
    docs_test, y_test, author_ids_test, original_tweet_lengths_test, test_ir_vars = \
        process_data_files.load_pan_data(
            PRESET['test_xmls_directory'], PRESET['test_truth_path'], False, None, exp_dict=exp_dict, exec_type='testing')
    ...
    return docs_train, docs_test, y_train, y_test, train_ir_vars, test_ir_vars
...
def load_datasets_tira_evaluation(test_dataset_main_directory, preset_key):
    ...
    xmls_directory_train = os.path.join(
        TRAINING_DATASET_MAIN_DIRECTORY, PRESET['xmls_subdirectory'])
    truth_path_train = os.path.join(
        TRAINING_DATASET_MAIN_DIRECTORY, PRESET['truth_subpath'])
    xmls_directory_test = os.path.join(
        test_dataset_main_directory, PRESET['xmls_subdirectory'])
    ...
    docs_train, y_train, author_ids_train, original_tweet_lengths_train = \
        process_data_files.load_pan_data(
            xmls_directory_train, truth_path_train, False, None, exp_dict=exp_dict, exec_type='training')

    # Load the PAN 2018 test dataset from the files into lists
    logger.info("Loading the %s test dataset...", PRESET['dataset_name'])
    docs_test, y_test, author_ids_test, original_tweet_lengths_test = \
        process_data_files.load_pan_data(
            xmls_directory_test, None, False, None, exp_dict=exp_dict, exec_type='testing')
    # ↳ Note: truth_path_test will not be provided to the participants. As a result, *truths_test* will be empty.
...
def extract_features(docs_train, docs_test, preset_key, train_ir_vars, test_ir_vars):
    """Extract features
    ...
    if (add_ir_variables and ir_variables_before_dim_reduction):
        ...
        if use_only_ir_variables:
            ...
            X_train_ngrams_tfidf = hstack(([X_train_ngrams_tfidf,csr_matrix(train_ir_vars)]))
        logger.info(
        ...
        if use_only_ir_variables:
            ...
            X_test_ngrams_tfidf = hstack(([X_test_ngrams_tfidf,csr_matrix(test_ir_vars)]))
        logger.info(
    ...
    if PRESET['perform_dimentionality_reduction']:
        ...
        logger.info(
            "@ %.2f seconds: Finished dimensionality reduction (LSA) on the training dataset", time.process_time())
        logger.info("Training set after dimensionality reduction (LSA) .shape = %s",
                    X_train_ngrams_tfidf_reduced.shape)

        X_test_ngrams_tfidf_reduced = svd.transform(X_test_ngrams_tfidf)
        logger.info(
            "@ %.2f seconds: Finished dimensionality reduction (LSA) on the test dataset", time.process_time())
        logger.info("Test set after dimensionality reduction (LSA) .shape = %s",
                    X_test_ngrams_tfidf_reduced.shape)
    ...
    if (add_ir_variables and (not ir_variables_before_dim_reduction)):
        ...
        logger.info("Training set .shape = %s",
                    X_train.shape)
        X_test = np.c_[X_test, test_ir_vars]
        logger.info(
            "@ %.2f seconds: Finished adding IR variables on the test dataset", time.process_time())
        logger.info("Test set .shape = %s",
                    X_test.shape)
    ...
    *flame_expressions_dict*
    ...
    for flame_index in DESIRED_FLAME_LEVELS:
        ...
        for expression in flame_expressions_dict[flame_index]:
            feature_names_offensive_words.append(
                "{} ({})".format(expression, flame_index))
    ...
    tfidf_transformer = TfidfTransformer(
        norm='l2', use_idf=False, sublinear_tf=False)
...
def count_offensive_words(docs, pickle_path_pattern=None):
    ...
    if (pickle_path is not None) and os.path.isfile(pickle_path):
        ...
        logger.info(
            'Function bypassed: The counts of offensive words was loaded from pickle "%s" instead.', pickle_path)
    ...
    *flame_expressions_dict*
    ...
    '''
    regex_objects_dict = {1: [], 2: [], 3: [], 4: [],
                          5: []}  # Create a dictionary of 5 empty lists
    for flame_index in range(1, 6):
        for expression in flame_expressions_dict[flame_index]:
    ...
    for author_index, merged_tweets_of_author in enumerate(preprocessed_docs):
        for flame_index in range(1, 6):
            for expression_index in range(len(flame_expressions_dict[flame_index])):
                ...
                list_of_matches = regex_objects_dict[flame_index][expression_index].findall(
                    merged_tweets_of_author)
                count = len(list_of_matches)
                # count = merged_tweets_of_author.count(expression)
                counts_of_expressions_dict[flame_index][author_index,
    ...
    logger.info(
        "@ %.2f seconds: Finished counting the occurrences of offensive words", time.process_time())
    ...
    if pickle_path is not None:
        ...
        logger.info(
            'The counts of offensive words was pickled to: "%s"', pickle_path)
...
def generate_pickle_path(input_object, pickle_path_pattern):
    ...
    if HASH_PLACEHOLDER not in pickle_path_pattern:
        raise ValueError(
            'The pickle path pattern should contain the hash placeholder, "%s".' % HASH_PLACEHOLDER)
    ...
    pickle_path = pickle_path_pattern.replace(
        HASH_PLACEHOLDER, hash_value_as_base32_encoded_string)
...
def cross_validate_model(clf, X_train, y_train):
    ...
    exp_dict['acc_cv_10'] = np.mean(scores)
    lb = LabelBinarizer()
    x_labels = lb.fit_transform(y_train).ravel()
    exp_dict['f1_cv_10'] = np.mean(cross_val_score(clf, X_train, x_labels, cv=skf, scoring='f1'))

    logger.info("@ %.2f seconds: Cross-validation finished",
                time.process_time())
...
def rank_importance_of_features(clf, feature_names, write_to_file):
    ...
    sorted_list_of_tuples = sorted(
        list_of_tuples, key=lambda tuple: abs(tuple[0]), reverse=True)
    ...
    sorted_feature_weights, sorted_feature_names = [
        list(a) for a in zip(*sorted_list_of_tuples)]
    ...
    plt.barh(range(PLOT_TOP),
             sorted_feature_weights[:PLOT_TOP], align='center')
...
def main_development():
    ...
    for presetKey in ("PAN18_English",
        ...
        docs_train, docs_test, y_train, y_test = load_datasets_development(
            presetKey)
        X_train, X_test, clf, feature_names = extract_features(
            docs_train, docs_test, presetKey)
...
def main_test():
    ...
    Every time the script runs, it will call this function.
    """
    mylogger.info('START OF CLF TRAIN AND TEST')
    ...
    for presetKey in ("PAN18_English",
        ...
        if use_dumped_features:
            ...
            docs_train, docs_test, y_train, y_test, train_ir_vars, test_ir_vars = dataset
        else:
        ...
        #      6.0, 8.0, 9.0, 10.0, 100.0, 10000.0, 99999999.0]
        # C = [1.0]
        C = [0.00001, 0.001, 0.002, 0.005, 0.01, 0.02, 0.03, 0.05, 0.07] + np.around(np.linspace(0.1,10,100,endpoint=True),2).tolist() + [100.0, 10000.0, 99999999.0]
        ...
        for c in C:
            ...
            # exp_dict['linear_svc_max_iter'] = str(150)
            # clf = LinearSVC(random_state=random_seed, C=c, max_iter=150)
            # # cross_validate_model(clf, X_train, y_train)
            # train_and_test_model(clf, X_train, y_train, X_test, y_test)
...
def main_tira_evaluation():
    ...
    logger.info("test_dataset_main_directory = %s",
                test_dataset_main_directory)
    ...
    for preset_key in ("PAN18_English", "PAN18_Spanish", "PAN18_Arabic"):
        ...
        docs_train, docs_test, y_train, author_ids_test =\
            load_datasets_tira_evaluation(
                test_dataset_main_directory, preset_key)

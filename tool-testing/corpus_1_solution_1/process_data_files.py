...
def get_ir_variable(text):
    ...
    ir_variables = {}
    time_query = 0.0
    testToolI = IndexToolManager(
        indexName=str(authorprof_db_name), top_k=global_exp_dict['ir_top_k'])
    initial = None
    final = None
    if (global_exp_dict['tool'] == 'arango'):
        initial = time.time()
        ir_variables = testToolI.arango_get_IR_variables(
            text, 'male', ignore_first_result=global_exp_dict['ignore_first_result'])
        final = time.time()
...
def get_ir_variables(text_list):
    ir_variables_of_this_author = []
    time_query_list = []
    global global_exp_dict

    if (global_exp_dict['tool'] == 'zettair'):
    ...
    # Run this with a pool of 5 agents having a chunksize of 3 until finished
        ...
        with Pool(processes=agents) as pool:
            ir_variables_of_this_author, time_query_list = \
                zip(*pool.map(get_ir_variable, text_list, chunksize))

    return ir_variables_of_this_author, time_query_list

def load_pan_data(xmls_directory, truth_path,
                  write_to_txt_files=False,
                  txts_destination_directory=None,
                  exp_dict={}, exec_type='training'):
    """Load PAN data
    ...
    for author_index, xml_filename in enumerate(xml_filenames):
        ...
        if not fnmatch.fnmatch(xml_filename, '*.xml'):
            logger.error("Encountered a non-XML file inside the directory: %s >>> The program will now exit.",
                         xml_filename)
            raise RuntimeError(
                'Encountered a non-XML file inside the directory: %s' % xml_filename)
        ...
        logger.info(f'{datetime.datetime.now().strftime("%Y%m%d-%H%M%S")} Author: {author_id}')
...
def load_truth(truth_path, author_ids):
    ...
    for i, row in enumerate(temp_sorted_author_ids_and_truths):
        ...
        if row[0] == author_ids[i]:
            ...
            raise RuntimeError(
                'Failed to sync the order of the Truth list and the Author ID list. Row number: %d' % i)
            # ↳ This is printf-style String Formatting.
...
def split_train_and_test_files(author_ids_train, author_ids_test, truths_train, truths_test, preset_key):
    ...
    xmls_destination_directory_train = os.path.join(
        TRAIN_DESTINATION_MAIN_DIR, PRESET['xmls_destination_subdirectory'])
    xmls_destination_directory_test = os.path.join(
        TEST_DESTINATION_MAIN_DIR, PRESET['xmls_destination_subdirectory'])
    truth_destination_path_train = os.path.join(
        TRAIN_DESTINATION_MAIN_DIR, PRESET['truth_destination_subpath'])
    truth_destination_path_test = os.path.join(
        TEST_DESTINATION_MAIN_DIR, PRESET['truth_destination_subpath'])
    ...
    for author_id in author_ids_train:
        shutil.copy(os.path.join(
            PRESET['xmls_source_directory'], author_id + ".xml"), xmls_destination_directory_train)
    for author_id in author_ids_test:
        shutil.copy(os.path.join(
            PRESET['xmls_source_directory'], author_id + ".xml"), xmls_destination_directory_test)
...
def write_predictions_to_xmls(author_ids_test, y_predicted, xmls_destination_main_directory, language_code):
    ...
    xmls_destination_directory = os.path.join(
        xmls_destination_main_directory, language_code)
    ...
    logger.info(
        "@ %.2f seconds: Finished writing the predictions to XML files", time.process_time())
...
def write_feature_importance_rankings_to_csv(sorted_feature_weights, sorted_feature_names, log_file_path):
    ...
    log_file_name_without_extension = os.path.splitext(
        os.path.basename(log_file_path))[0]
    CSV_PATH = os.path.join(
        log_file_directory, log_file_name_without_extension + "; Feature importance rankings.csv")
...
def write_iterable_to_csv(iterable, iterable_name, log_file_path):
    ...
    log_file_name_without_extension = os.path.splitext(
        os.path.basename(log_file_path))[0]
    CSV_PATH = os.path.join(
        log_file_directory, log_file_name_without_extension + "; " + iterable_name + ".csv")
